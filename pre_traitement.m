...
if all(isfield(donnees.zone,{'P_int','P_out','P_surf','P_sys_air','P_sys_conv'}))
    ...
    resultats.bilan_air.P_int=sum(donnees.zone.P_int(:,zones_etude),2)*24/(1000*surf_zones);
    %Zone Air Balance Outdoor Air Transfer Rate [W](Daily)
    resultats.bilan_air.P_out=sum(donnees.zone.P_out(:,zones_etude),2)*24/(1000*surf_zones);
    %Zone Air Balance Surface Convection Rate [W](Daily)
    resultats.bilan_air.P_surf=sum(donnees.zone.P_surf(:,zones_etude),2)*24/(1000*surf_zones);
    %Zone Air Balance System Air Transfer Rate [W](Daily)
    %Zone Air Balance System Convective Gains Rate [W](Daily)
    resultats.bilan_air.P_sys=(sum(donnees.zone.P_sys_air(:,zones_etude),2)+sum(donnees.zone.P_sys_conv(:,zones_etude),2))*24/(1000*surf_zones);
end
...
if isfield(donnees,'E_elec_tot')
    ...
    resultats.conso.Elec_tot=sum(donnees.E_elec_tot,1)/(3600*1000*surf_zones); %x [J] => x/(3600*1000) [kWh]
...
if isfield(donnees.zone, 'E_heat')    
    if isfield(donnees.zone.E_heat, 'Sensible')
        ...
        resultats.energie.heat.Sensible = sum(donnees.zone.E_heat.Sensible(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.heat.Sensible = 0;
    end
    if isfield(donnees.zone.E_heat, 'Latent')
        %  ZONE% IDEAL LOADS AIR SYSTEM:Zone Ideal Loads Zone Latent Heating Rate [W](Hourly)     
        resultats.energie.heat.Latent = sum(donnees.zone.E_heat.Latent(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.heat.Latent = 0;
    end
    if isfield(donnees.zone.E_heat, 'Total')
        %  ZONE% IDEAL LOADS AIR SYSTEM:Zone Ideal Loads Zone Total Heating Rate [W](Hourly)
        resultats.energie.heat.Total = sum(donnees.zone.E_heat.Total(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.heat.Total = 0;
    end
end
...
if isfield(donnees.zone, 'E_cool')    
    if isfield(donnees.zone.E_cool, 'Sensible')
        ...
        resultats.energie.cool.Sensible = sum(donnees.zone.E_cool.Sensible(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.cool.Sensible = 0;
    end
    if isfield(donnees.zone.E_cool, 'Latent')
        ...
        resultats.energie.cool.Latent = sum(donnees.zone.E_cool.Latent(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.cool.Latent = 0;
    end
    if isfield(donnees.zone.E_cool, 'Total')
        ...
        resultats.energie.cool.Total = sum(donnees.zone.E_cool.Total(:,zones_etude),2)/(1000); %[kWh]
    else
        resultats.energie.cool.Total = 0;
    end
end
...
if isfield(donnees.surface, 'E_win_ray')
    ...
    resultats.vitrages.card_ray= [sum( donnees.surface.E_win_ray(:,fenetres(:,1)) ,2)/(3600*1000*win_surf_nord),...
    ...
    if all(isfield(donnees.surface, {'E_win_gain', 'E_win_loss'}))
        ...
        resultats.vitrages.card.gain= [sum( donnees.surface.E_win_gain(:,fenetres(:,1)) ,2)/(3600*1000*win_surf_nord),...
        ...
        resultats.vitrages.card.loss= -[sum( donnees.surface.E_win_loss(:,fenetres(:,1)) ,2)/(3600*1000*win_surf_nord),...
...
if isfield(donnees.zone,'Tas')
    resultats.humdite.T_int  = (donnees.zone.Tas(:,zones_etude)*volumes_zones')/sum(volumes_zones);
end
if isfield(donnees.zone,'RH')
    resultats.humdite.RH_int = (donnees.zone.RH(:,zones_etude)*volumes_zones')/sum(volumes_zones);
end
if isfield(donnees.zone,'w')
    resultats.humdite.w_int = (donnees.zone.w(:,zones_etude)*volumes_zones')/sum(volumes_zones);
end
...
if isfield(donnees,'simple') && isfield(donnees.simple,'T_ext') && length(donnees.simple.T_ext)>=7 &&  isfield(donnees.zone,'Toperative')
    ...
    resultats.conf_adap.T_optimale = (T_sup+T_inf)/2; %(hyp. perso)
    ...
    resultats.conf_adap.categories= (T_z>=resultats.conf_adap.T_optimale).*((T_z>T_sup+4)+(T_z>T_sup+3)+(T_z>T_sup+2)+1)...
                                   -(T_z< resultats.conf_adap.T_optimale).*((T_z<T_inf-4)+(T_z<T_inf-3)+(T_z<T_inf-2)+1);

    % Ecart avec la CatI en °C
    resultats.conf_adap.ecartCatI = [max(0,T_z-(T_sup+2)), min(0,T_z-(T_inf-2))];
...
if isfield(geometrie,'lightzones') && any(geometrie.lightzones{:,1}) && extract_lum
    ...
    resultats.eclairement.nbs_heures=sum(donnees.eclairement.actif',2); %#ok<UDIM>
    ...
    [resultats.eclairement.max_j,index] = max(resultats.eclairement.moy_h',[],2); %#ok<UDIM>
