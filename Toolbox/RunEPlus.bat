@echo off
 : Version modifiee de RunEPlus.bat :
 : La structure du programme a été conservee au maximum
 : La variable 'Ep_dir' permet de specifier le repertoire de localisation par defaut
 : Un repertoire au nom du fichier idf est cree et est defini comme repertoire de travail
 : A la suite des simulations un retour au repertoire initial est réalisé
...
:                  "nolimit" if unlimited (used when calling readVarsESO)
 ...
 set Ep_dir="D:\EnergyPlusV7-1-0\"
 set program_path=%Ep_dir%
 ...
 : Mémorisation du chemin des fichiers idf/imf
 set idf_path=%~dp1
 : Définition d'un répertoire de travail au nom du fichier
 : input_path et output_path étant vide tous les fichiers seront dans le rep courant, CD
 set work_path=%~dpn1
 IF NOT EXIST "%work_path%". MKDIR "%work_path%"
 cd %work_path%
 set post_proc=%program_path%PostProcess\
 set weather_path=%program_path%WeatherData\
 if EXIST "%idf_path%%2.epw" set weather_path=%idf_path%
 if EXIST "%2.epw" set weather_path=
 set pausing=N
 set maxcol=250
...
if exist "%idf_path%%~1.imf" copy "%idf_path%%~1.imf" in.imf
...
if not exist in.idf copy "%idf_path%%~1.idf" In.idf
...
:exe
 ...
 echo Input File  : %idf_path%%~1.idf
 echo Output Files: %work_path%
 ...
 : Force le nombre de coeurs a 1
SET EP_OMP_NUM_THREADS=1
