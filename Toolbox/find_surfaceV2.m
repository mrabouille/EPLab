function [varargout]=find_surfaceV2(file,sorties)
...
%}
    ...
    if niveau<2 && ~isempty(strfind(ligne,' People Internal Gains Nominal,'))
        niveau=1;
        A = textscan(ligne,'%s', 'delimiter',',');
        A = A{1}(2:end);
...
%}
    ...
    if niveau<4 && ~isempty(strfind(ligne,' ZoneInfiltration Airflow Stats Nominal,'))
        niveau=3;
        A = textscan(ligne,'%s', 'delimiter',',');
        A = A{1}(2:end);
...
%}
    ...
    if niveau<5 && ~isempty(strfind(ligne,' ZoneVentilation Airflow Stats Nominal,'))
        niveau=4;
        A = textscan(ligne,'%s', 'delimiter',',');
        A = A{1}(2:end);
...
%}
    ...
    if isempty(strfind(ligne,'! <Zone Surfaces>'))
        ligne  = fgets(fid);
        continue
    else
        while isempty(strfind(ligne,'Zone Surfaces,'))
            ligne  = fgets(fid);
        end
...
if isempty(geometrie.surfaces)
    error(['Probleme lors de la lecture des données géométriques.\nVérifiez le fichier :\t%s\n'],...
    	fullfile(file));
end
