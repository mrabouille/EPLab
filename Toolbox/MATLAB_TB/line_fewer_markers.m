...
if mod(length(varargin),2)
    if ischar(varargin{1})
      ...
      [varargInNoMk,varargInNoMkNoLn,lm,ms,mfc,LockOnMax,Spacing,LegendLine,Offset] = parseargsLineSpec(linspec,extraArgs);
    else
      error('odd sized [param | val] list, missing one param ?');
    end
else
      [varargInNoMk,varargInNoMkNoLn,lm,ms,mfc,LockOnMax,Spacing,LegendLine,Offset] = parseargs(varargin{:});  
end
...
function [varargInNoMk,varargInNoMkNoLn,lm,ms,mfc,LockOnMax,Spacing,LegendLine,Offset] = parseargs(varargin)
...
while arg_index <= length(varargin)
    ...
    elseif strcmp(arg,'Offset')   || strcmp(arg,'offset') 
        Offset          = varargin{arg_index+1};
    else
...
% varargInNoMk     = list of property pairs, marker specs removed 
% varargInNoMkNoLn = list of property pairs, marker specs and line specs removed 
function [varargInNoMk,varargInNoMkNoLn,lm,ms,mfc,LockOnMax,Spacing,LegendLine,Offset] = parseargsLineSpec(linspec, extraArgs)
%          b     blue          .     point              -     solid
...
[extraArgs1,unused,lm2,ms,mfc,LockOnMax,Spacing,LegendLine,Offset] = parseargs(extraArgs{:});
